    ...
    try {
      ...
      await firestore()
        ...
        .then(querySnapshot => {
          // console.log('Total Posts: ', querySnapshot.size);
          querySnapshot.forEach(doc => {
            const {postid, uid, post, postImg, postTime, tag, likes, comments} =
              doc.data();
            list.push({
              id: doc.id,
              uid,
    ...
    try {
      ...
      await firestore()
        ...
        .then(querySnapshot => {
          ...
          querySnapshot.forEach(doc => {
            const {uid, post, postImg, postTime, tag, likes, comments} =
              doc.data();
            list.push({
              id: doc.id,
              uid,
...
>>>>>>> 10734237e94c9a5d7f974562fb0d28729cecf45a
  ...
  const RenderCard = ({item}) => {
    return (
      <TouchableOpacity
        onPress={() =>
          navigation.navigate('SearchSnsScreen', {
            tag: item.tag,
            uid: item.uid,
            ...
            {Bestposts?.map((row, idx) => {
              return (
                <View>
                  <TouchableOpacity
                    onPress={() =>
                      navigation.navigate('SerachBestSnsScreen', {
                        uid: row.uid,
                        postimg: row.postImg,
                        post: row.post,
                    ...
                    <Image
                      source={{uri: row.postImg}}
                      style={{width: 200, height: 150, marginLeft: 10}}></Image>
                  </TouchableOpacity>
                </View>
              );
            })}
        ...
        <View style={{flexDirection: 'row', marginBottom: 5}}>
          <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
            ...
            <TouchableOpacity
              ...
              onPress={() => TagList(userData ? userData.Lsearch : '동물')}>
            ...
            <TouchableOpacity
              style={styles.button2}
              onPress={() =>
                AllBestPosts(userData ? userData.InterSearch : '동물')
              }>
